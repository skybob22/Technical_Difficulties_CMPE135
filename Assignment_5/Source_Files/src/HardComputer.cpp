 ...
 */
ComputerDifficulty::Difficulty HardComputer::getDifficulty(){
    return ComputerDifficulty::Difficulty::Hard;
}

/**
 * @brief Gets the prediction of what the human will play next
 * @return The predicted value or a None type if no prediction could be made
 */
PlayerChoice::Choice HardComputer::getPrediction(){
//Use ML algorithm, check if current pattern exists in map
    ...
    if(pattMatch == historyMap.end() || pattMatch->second.empty()){
        ...
        return PlayerChoice::Choice::None;
    }
...
PlayerChoice::Choice HardComputer::getPlayerChoice(){
    PlayerChoice::Choice predictedValue = getPrediction();
    if(predictedValue == PlayerChoice::Choice::None){
    ...
    for(PlayerChoice::Choice choice : PlayerChoice::getEnumList()){
        if(PlayerChoice::evaluateMatchup(predictedValue,choice) > 0){
